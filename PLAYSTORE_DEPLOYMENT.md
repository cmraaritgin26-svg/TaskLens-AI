...
- Data safety form: disclose health and fitness data, journal text, voice
  dictation/transcripts, app activity/tasks, diagnostics if any are added later,
  and whether data is shared with configured AI services.
...
- Permissions declaration: microphone, notifications, biometric/device unlock,
  vibration, internet, and full-screen/crisis alert behavior.
- Content rating.
...
```text
Health & Task Tracker helps you keep daily tasks, nutrition, water, vitals, symptoms, mood, and journal entries in one private tracker. The app includes weekly task progress, history charting, journal logs, dictation review, reminders, and optional AI-assisted summaries when you configure cloud AI.
...
- Optional cloud AI can send health information, journal text, mood/symptom
  notes, nutrition/vitals, tasks, and dictation transcripts over the internet
  after the user allows it in setup/settings and configures an AI service.
- Voice recordings are not saved by the app.
...
- Verify first-run setup, AI internet-sharing acknowledgement, app lock,
  dictation, journal delete/delete all, crisis alert behavior, charting, and
  export/import.
